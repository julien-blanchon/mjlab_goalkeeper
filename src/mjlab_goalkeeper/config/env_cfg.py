...
class ObservationCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        left_hand_position: ObsTerm = term(
            ObsTerm,
            func=partial(mdp.hand_position, hand_name="left_wrist_yaw_link"),
        )
        right_hand_position: ObsTerm = term(
            ObsTerm,
            func=partial(mdp.hand_position, hand_name="right_wrist_yaw_link"),
        )
        ...
        robot_ball_contact: ObsTerm = term(
            ObsTerm,
            func=partial(mdp.robot_ball_contact, threshold=0.5),
        )
...
@dataclass
class RewardCfg:
    # Removed velocity tracking rewards - not needed for goalkeeper
    ...
    alive: RewardTerm = term(
        RewardTerm,
        func=mdp.is_alive,
        weight=1.0,  # Constant +1.0 per step for staying upright
    )
...
class UnitreeG1FlatEnvCfg_PLAY(UnitreeG1FlatEnvCfg):
    """Configuration for Unitree G1 robot on flat terrain (play mode)."""
